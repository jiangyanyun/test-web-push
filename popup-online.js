    ...
    createView: function (template) {
      ...
      if (template.layout.align) {
        ...
        if (template.isRoot) {
          ...
          style = style + "width:100%;height:100%;overflow-y:auto;box-sizing: border-box;";
  ...
  popup.setPara = function(para){
    ...
    if(!_.isObject(popup.info.popup_listener)){
      ...
      if(!_.isString(popup_listener.openlink)){
        ...
        // openlink是字符串，但是value不是auto或customize，直接用默认的customize
  ...
  */
  ...
  popup.RuleCheck.prototype.checkConvert = function(){
    ...
    if(_.isObject(this.plan.is_in_convert_window) && this.plan.is_in_convert_window.expire_time > this.current_time ) {
      ...
      salog('--不存在转化窗口或者窗口超时已经过期 - 满足',this.plan.is_in_convert_window);      
